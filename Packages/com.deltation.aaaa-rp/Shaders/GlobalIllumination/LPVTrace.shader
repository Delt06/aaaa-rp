    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            float4 Frag(const Varyings IN) : SV_Target
            {
                ...
                const float3       gbufferPositionWS = ComputeWorldSpacePosition(IN.texcoord, deviceDepth, UNITY_MATRIX_I_VP);
                const GBufferValue gbufferValue = SampleGBuffer(IN.texcoord);
                ...
                for (uint lightIndex = 0; lightIndex < directionalLightCount; ++lightIndex)
                {
                    ...
                    if (sliceRange.y > 0)
                    {
                        ...
                        const uint                 selectedCascadeIndex = ComputeCascadeIndex(gbufferPositionWS, cascadeSelectionParameters);
                        ...
                        )
                        {
                            ...
                            const float3 shadowCoords = TransformWorldToShadowCoords(gbufferPositionWS, selectedCascadeSlice.WorldToShadowCoords,
                                                       isPerspective);
                            ...
                            {
                                const float2   offsetShadowCoords = shadowCoords.xy + radiusModifier * R_MAX * float3(Samples[i], 0);
                                const RsmValue rsmValue = SampleRsmValue(selectedCascadeSlice, offsetShadowCoords);

                                const float3 offset = gbufferPositionWS - rsmValue.positionWS;
                                const float dist2 = max(FLT_MIN, dot(offset, offset));
                                const float  SNdotO = max(0, (dot(rsmValue.normalWS, offset)));
                                const float  NdotO = max(0, (dot(gbufferValue.normalWS, -offset)));
                                float3       sampleContribution = rsmValue.flux * SNdotO * NdotO;
                                sampleContribution *= Samples[i].x * Samples[i].x / (dist2 * dist2);

                                lightContribution += sampleContribution;
                            }

                            lighting += lightContribution * lightColor;
                        }
                    }
                }

                return float4(lighting * INTENSITY + aaaa_AmbientIntensity * SampleDiffuseIrradiance(gbufferValue.normalWS), 1);
            }
            ENDHLSL
        }
    }
