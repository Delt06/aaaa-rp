...
float SpecularAO_Cones(float visibility, const float3 bentNormalWS, const float3 R, const float NoV, float roughness)
{
...
float ComputeSpecularAO(float visibility, const float3 bentNormalWS, const float3 R, const float NoV, float roughness) {
    ...
    return SpecularAO_Cones(visibility, bentNormalWS, R, NoV, roughness);
    #else
    return 1.0;
    #endif
}
