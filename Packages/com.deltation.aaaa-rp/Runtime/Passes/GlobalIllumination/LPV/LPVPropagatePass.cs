...
namespace DELTation.AAAARP.Passes.GlobalIllumination.LPV
{
    public class LPVPropagatePass : AAAARenderPass<LPVPropagatePass.PassData>
    {
        ...
        protected override void Setup(RenderGraphBuilder builder, PassData passData, ContextContainer frameData)
        {
            ...
            passData.SkyOcclusionPropagationIntensity = lpvVolumeComponent.SkyOcclusionPropagationIntensity.value;
            passData.SkyOcclusion = passData.SkyOcclusionPropagationIntensity > 0.0f && lpvData.SkyOcclusionTexture.IsValid();
            ...
            passData.SkyOcclusionTexture = passData.SkyOcclusion ? builder.ReadWriteTexture(lpvData.SkyOcclusionTexture) : default;
            passData.SkyOcclusionTempTexture = passData.SkyOcclusion
                ? builder.CreateTransientTexture(new TextureDesc(lpvData.SkyOcclusionTextureDesc)
                    {
                        name = "LPV_" + nameof(PassData.SkyOcclusionTempTexture),
                    }
                )
                : default;
        ...
        protected override void Render(PassData data, RenderGraphContext context)
        {
            ...
            CoreUtils.SetKeyword(context.cmd, _computeShader, AAAALPVCommon.ShaderKeywords.SKY_OCCLUSION, data.SkyOcclusion);
            if (data.SkyOcclusion)
            {
                context.cmd.SetComputeFloatParam(_computeShader, ShaderIDs._SkyOcclusionPropagationIntensity, data.SkyOcclusionPropagationIntensity);
            }

            for (int i = 0; i < data.PassCount; ++i)
            {
                CoreUtils.SetKeyword(context.cmd, _computeShader, ShaderKeywords.FIRST_STEP, i == 0);
                RenderPass(data, context, data.Grid, data.LPVTempGrid, data.SkyOcclusionTexture, data.SkyOcclusionTempTexture);
                CoreUtils.SetKeyword(context.cmd, _computeShader, ShaderKeywords.FIRST_STEP, false);
                RenderPass(data, context, data.LPVTempGrid, data.Grid, data.SkyOcclusionTempTexture, data.SkyOcclusionTexture);
            }
        }

        private void RenderPass(PassData data, RenderGraphContext context, in GridTextureSet source, in GridTextureSet destination,
            TextureHandle sourceSkyOcclusion, TextureHandle destinationSkyOcclusion)
        {
            ...
            if (data.SkyOcclusion)
            {
                context.cmd.SetComputeTextureParam(_computeShader, KernelIndex, ShaderIDs._SourceSkyOcclusion, sourceSkyOcclusion);
                context.cmd.SetComputeTextureParam(_computeShader, KernelIndex, ShaderIDs._DestinationSkyOcclusion, destinationSkyOcclusion);
            }
        ...
        public class PassData : PassDataBase
        {
            ...
            public float SkyOcclusionPropagationIntensity;
            public TextureHandle SkyOcclusionTempTexture;
        ...
        private static class ShaderIDs
        {
            ...
            public static readonly int _SourceSkyOcclusion = Shader.PropertyToID(nameof(_SourceSkyOcclusion));
            ...
            public static readonly int _DestinationSkyOcclusion = Shader.PropertyToID(nameof(_DestinationSkyOcclusion));
            ...
            public static readonly int _SkyOcclusionPropagationIntensity = Shader.PropertyToID(nameof(_SkyOcclusionPropagationIntensity));
